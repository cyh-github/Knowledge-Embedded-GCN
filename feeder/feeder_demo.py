...
class Feeder(torch.utils.data.Dataset):
    ...
    def load_data(self, mmap):
        ...
        if mmap:
            ...
            self.org_data = np.load(self.org_data_path, mmap_mode='r')
        else:
            self.joint_data = np.load(self.joint_data_path)
            self.part_data = np.load(self.part_data_path)
            self.org_data = np.load(self.org_data_path)
    ...
    def __getitem__(self, index):
        ...
        org_data_numpy = np.array(self.org_data[index])
        label = self.label[index]

        return joint_data_numpy, part_data_numpy, org_data_numpy, label
