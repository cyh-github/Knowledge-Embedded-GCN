...
class Processor(IO):
        ...
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if ((epoch + 1) % self.arg.eval_interval == 0) or (
                    ...
                    self.io.print_log('*****Test result:')
                    test_accuracy = self.test(test_data='test')
                    if test_accuracy >= Best_accuracy:
                        Best_accuracy = test_accuracy
                        Best_epoch = epoch
                        filename = 'epoch{:.2f}_{}model.pt'.format(Best_accuracy*100, Best_epoch+1)
                        self.io.save_model(self.model, filename)
                    self.io.print_log('###########Best accuracy:{:.2f}% in epoch{}'.format(Best_accuracy*100, Best_epoch+1))
                    self.io.print_log('*****Train result:')
                    self.test(test_data='train')
                    self.io.print_log('Done.')
        # test phase
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('-c', '--config', default='./config/st_gcn/SBU/train_3A_set1.yaml', help='path to the configuration file')
