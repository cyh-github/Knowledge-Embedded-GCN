...
from utils.ntu_read_skeleton import read_xyz, read_pixel
...
def gendata(data_path,
    ...
    for i, s in enumerate(sample_name):
        ...
        fp[i, :, 0:data.shape[1], :, :] = np.concatenate((data[:,:,:,:,0], data[:,:,:,:,1]),axis=-1)

        pixel_data = read_pixel(os.path.join(data_path, s), max_body=max_body, num_joint=num_joint)
        fp_pixel[i, :, 0:data.shape[1], :, :] = np.concatenate((pixel_data[:,:,:,:,0], pixel_data[:,:,:,:,1]),axis=-1)
    end_toolbar()
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--ignored_sample_path',
        default='./data/NTU_2p_data/samples_with_missing_skeletons.txt')
